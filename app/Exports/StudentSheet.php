...
class StudentSheet extends BaseSheet implements FromGenerator
{
  ...
  {
    return ['Student ID', 'Noreg', 'Nama', 'Asal Sekolah', 'Tingkat Kelas', 'Kelas di GO', 'Level', 'Unit ID', 'Kota', 'Unit', 'Nilai ID', 'Mapel', 'Jenis Nilai', 'Tanggal Ujian', 'Validasi PT', 'Testimoni', 'Keterangan', 'STATUS PUBLIKASI (SUDAH/BELUM)'];
  }
  protected function base($s): array
  {
    ...
    return [$s->id, $s->noreg, $s->nama_siswa, $s->asal_sekolah, $s->tingkat_kelas, $s->kelas_di_go, $s->level, $s->unit_id, $s->unit->kota, $s->unit->nama_unit, $n?->id, $n?->mapel, $n?->jenis_nilai, $n?->tanggal_ujian?->format('d/m/Y'), $n?->tanggal_validasi_pt?->format('d/m/Y'), $n?->status_testimoni, $n?->keterangan, $publicationStatus];
  }
