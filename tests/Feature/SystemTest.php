...
class SystemTest extends TestCase {
 ...
 protected function setUp():void {
  ...
  $this->a=Unit::create(['kota'=>'Kota A','nama_unit'=>'Unit A']);$this->b=Unit::create(['kota'=>'Kota B','nama_unit'=>'Unit B']);
 ...
 private function student(Unit $u,string $n):Student{return Student::create(['noreg'=>$n,'nama_siswa'=>'Siswa '.$n,'asal_sekolah'=>'Sekolah','kelas_di_go'=>'12','unit_id'=>$u->id]);}
 private function grade(Student $s,string $status='SUDAH'):Nilai100{return $s->nilai100()->create(['jenis_nilai'=>'UH','tanggal_ujian'=>'2026-01-01','tanggal_validasi_pt'=>'2026-01-02','status_testimoni'=>$status]);}
 ...
 public function test_admin_cannot_forge_unit_on_create():void {$this->actingAs($this->admin)->post('/students',['noreg'=>'0099','nama_siswa'=>'X','asal_sekolah'=>'X','kelas_di_go'=>'12','unit_id'=>$this->b->id])->assertSessionHasErrors('unit_id');$this->assertDatabaseMissing('students',['noreg'=>'0099']);}
 ...
 public function test_grade_is_unique_and_other_unit_grade_rejected():void {
  $data=['student_id'=>$this->own->id,'jenis_nilai'=>'UH','tanggal_ujian'=>'2026-01-01','tanggal_validasi_pt'=>'2026-01-02','status_testimoni'=>'SUDAH'];
 ...
 public function test_grade_date_order_rejected():void {$this->actingAs($this->admin)->post('/nilai',['student_id'=>$this->own->id,'jenis_nilai'=>'UH','tanggal_ujian'=>'2026-01-03','tanggal_validasi_pt'=>'2026-01-02','status_testimoni'=>'SUDAH'])->assertSessionHasErrors('tanggal_validasi_pt');}
 ...
 public function test_grade_cannot_be_removed_or_downgraded_with_publications():void {
  ...
  $this->put('/nilai/'.$this->own->id,['jenis_nilai'=>'UH','tanggal_ujian'=>'2026-01-01','tanggal_validasi_pt'=>'2026-01-02','status_testimoni'=>'BELUM'])->assertSessionHasErrors('status_testimoni');
 }
 ...
 public function test_resume_does_not_multiply_metrics_and_counts_unique_students():void {
  ...
  $this->assertEquals(1,$r->jumsis);$this->assertEquals(1,$r->dipublikasi);$this->assertEquals(1,$r->nilai_uh_pts);$this->assertEquals(4,$r->ig_post);$this->assertEquals(20,$r->ig_view);$this->assertEquals(2,$r->tt_post);$this->assertEquals(3,$r->wa_kirim);
 ...
 public function test_export_scope_sheet_names_text_and_raw_event_rows():void {
  ...
  $raw=iterator_to_array($sheets[1]->generator());$this->assertCount(2,$raw);$this->assertSame('0001',$raw[0][1]);$this->assertCount(27,$raw[0]);
 ...
 public function test_import_rejects_cross_unit_duplicate_and_has_atomic_rollback():void {
  $rows=collect([collect(['Noreg','Nama','Asal Sekolah','Kelas','Unit']),collect(['0010','Valid','School','12',$this->a->id]),collect(['0011','Foreign','School','12',$this->b->id])]);
  ...
  $rows=collect([collect(['Noreg','Nama','Asal Sekolah','Kelas','Unit']),collect(['0001','Duplicate','School','12',$this->a->id])]);
 ...
 public function test_real_xlsx_upload_preserves_leading_zero_noreg():void {
  $book=new Spreadsheet;$sheet=$book->getActiveSheet();$sheet->fromArray([['Noreg','Nama','Asal Sekolah','Kelas','Unit'],['','Imported','School','12',$this->a->id]]);$sheet->setCellValueExplicit('A2','000010',\PhpOffice\PhpSpreadsheet\Cell\DataType::TYPE_STRING);
  $path=tempnam(sys_get_temp_dir(),'import').'.xlsx';(new Xlsx($book))->save($path);
  try{$this->actingAs($this->admin)->post('/students/import',['file'=>new UploadedFile($path,'students.xlsx','application/vnd.openxmlformats-officedocument.spreadsheetml.sheet',null,true)])->assertSessionHasNoErrors();$this->assertDatabaseHas('students',['noreg'=>'000010','unit_id'=>$this->a->id]);}finally{if(file_exists($path))unlink($path);}
 }
